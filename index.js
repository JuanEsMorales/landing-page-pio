...
app.get('/jeans/:id', (req, res) => {
  ...
  if (req.query.promotion) {
    const { id, name, description, image, price, previuosPrice } = jeansOnPromotion.find(jeans => jeans.id === Number(req.params.id));
    let category = 'jeans';
    return res.render('product', {
      ...
      previuosPrice,
      price,
      sizesAvailable: sizesAvailableJeans,
      relatedProducts: jeansSecondSlide,
      category,
      promotion: true,
    });
  }

  const { id, name, description, image, secondImage, price } = jeansSecondSlide.find(jeans => jeans.id === Number(req.params.id));
  let category = 'jeans';
  res.render('product', {
    id,
    name,
    ...
    price,
    sizesAvailable: sizesAvailableJeans,
    relatedProducts: jeansSecondSlide.filter(jeans => jeans.id !== id),
    category,
  });
});

app.get('/jackets/:id', (req, res) => {
  const { id, name, description, image, secondImage, price } = jacketsSecondSlide.find(jackets => jackets.id === Number(req.params.id));
  ...
  res.render('product', {
    ...
    secondImage,
    price,
    sizesAvailable: sizesAvailableJackets,
    relatedProducts: jacketsSecondSlide.filter(jackets => jackets.id !== id),
    category,
  });
});
